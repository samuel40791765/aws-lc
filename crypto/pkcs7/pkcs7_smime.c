...
static ASN1_VALUE *b64_read_asn1(BIO *bio, const ASN1_ITEM *it) {
  ...
  ASN1_VALUE *val = (ASN1_VALUE *)ASN1_item_d2i_bio(it, bio, NULL);
  if (!val) {
...
// mime_parse_hdr is the big one: parse MIME header lines up to message body.
...
static ASN1_VALUE *SMIME_read_ASN1(BIO *bio, BIO **bcont, const ASN1_ITEM *it)
{
    ...
    if ((headers = mime_parse_hdr(bio)) == NULL) {
        OPENSSL_PUT_ERROR(ASN1, ASN1_R_MIME_PARSE_ERROR);
        return NULL;
    }
    ...
    if ((hdr = mime_hdr_find(headers, "content-type")) == NULL
        || hdr->value == NULL) {
        sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
        OPENSSL_PUT_ERROR(ASN1, ASN1_R_NO_CONTENT_TYPE);
        return NULL;
    }
    ...
    if (strcmp(hdr->value, "multipart/signed") == 0) {
        ...
        if (!prm || !prm->param_value) {
            sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
            OPENSSL_PUT_ERROR(ASN1, ASN1_R_NO_MULTIPART_BOUNDARY);
            return NULL;
        }
        ret = multi_split(bio, prm->param_value, &parts);
        sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
        if (!ret || (sk_BIO_num(parts) != 2)) {
            OPENSSL_PUT_ERROR(ASN1, ASN1_R_NO_MULTIPART_BODY_FAILURE);
        ...
        if ((headers = mime_parse_hdr(asnin)) == NULL) {
            OPENSSL_PUT_ERROR(ASN1, ASN1_R_MIME_SIG_PARSE_ERROR);
            sk_BIO_pop_free(parts, BIO_vfree);
            return NULL;
        }
        ...
        if ((hdr = mime_hdr_find(headers, "content-type")) == NULL
            || hdr->value == NULL) {
            sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
            OPENSSL_PUT_ERROR(ASN1, ASN1_R_NO_SIG_CONTENT_TYPE);
            sk_BIO_pop_free(parts, BIO_vfree);
            return NULL;
        }
        ...
        if (strcmp(hdr->value, "application/x-pkcs7-signature") &&
            ...
            OPENSSL_PUT_ERROR(ASN1, ASN1_R_SIG_INVALID_MIME_TYPE);
        ...
        if ((val = b64_read_asn1(asnin, it)) == NULL) {
            OPENSSL_PUT_ERROR(ASN1, ASN1_R_ASN1_SIG_PARSE_ERROR);
            sk_BIO_pop_free(parts, BIO_vfree);
            return NULL;
        }
    ...
    if (strcmp(hdr->value, "application/x-pkcs7-mime") &&
        ...
        OPENSSL_PUT_ERROR(ASN1, ASN1_R_INVALID_MIME_TYPE);
        ERR_add_error_data(2, "type: ", hdr->value);
        sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
        return NULL;
    }
    ...
    if ((val = b64_read_asn1(bio, it)) == NULL) {
        OPENSSL_PUT_ERROR(ASN1, ASN1_R_ASN1_PARSE_ERROR);
        return NULL;
    }
    return val;
