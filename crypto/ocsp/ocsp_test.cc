...
TEST(OCSPTest, TestNotRequestedOCSP) {
  ...
  X509_STORE_add_cert(trust_store.get(),
                      LoadX509fromPEM(ca_cert).get());
  bssl::UniquePtr<STACK_OF(X509)> server_cert_chain =
      CertsToStack({LoadX509fromPEM(server_ecdsa_cert).get(),
                    LoadX509fromPEM(ca_cert).get()});
...
// Test OCSP response signed by the wrong responder certificate, but the requested
// certificate was signed. (however this incorrect OCSP responder certificate is a
...
TEST(OCSPTest, TestGoodOCSP_SHA256) {
  bssl::UniquePtr<OCSP_BASICRESP> basic_response;
  ExtractBasicOCSP(ocsp_response_sha256_der, OCSP_RESPONSE_STATUS_SUCCESSFUL, &basic_response);
  ASSERT_TRUE(basic_response);
  ...
  int reason = 0;
  // Convert issuer certificate to |OCSP_CERTID|
  bssl::UniquePtr<OCSP_CERTID> cert_id = bssl::UniquePtr<OCSP_CERTID>(OCSP_cert_to_id(EVP_sha256(), subject, issuer));
  ASSERT_TRUE(cert_id);
...
TEST(OCSPTest, TestRevokedOCSP_SHA256) {
  bssl::UniquePtr<OCSP_BASICRESP> basic_response;
  ExtractBasicOCSP(ocsp_revoked_response_sha256_der, OCSP_RESPONSE_STATUS_SUCCESSFUL, &basic_response);
  ASSERT_TRUE(basic_response);
...
TEST(OCSPTest, TestNotValidResponseOCSP_SHA256) {
  bssl::UniquePtr<OCSP_BASICRESP> basic_response;
  ExtractBasicOCSP(ocsp_response_wrong_signer_sha256_der, OCSP_RESPONSE_STATUS_SUCCESSFUL, &basic_response);
  ASSERT_TRUE(basic_response);
  ...
  X509_STORE_add_cert(trust_store.get(),
                      LoadX509fromPEM(ca_cert).get());
  bssl::UniquePtr<STACK_OF(X509)> server_cert_chain =
      CertsToStack({LoadX509fromPEM(server_cert).get(),
                    LoadX509fromPEM(ca_cert).get()});
