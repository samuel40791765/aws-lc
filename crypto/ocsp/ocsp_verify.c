 ...
 */
#define GUARD_CLEANUP(x, cleanup, error)   do {if ( !((x) >= S2N_OCSP_SUCCESS) ) { GUARD(cleanup); BAIL_POSIX(error); }} while (0)

static int basic_verify_cleanup(STACK_OF(X509) *chain, OCSP_BASICRESP *bs, STACK_OF(X509) *certs, STACK_OF(X509) *untrusted);

#define S2N_OCSP_MISMATCH 1
...
int OCSP_basic_verify(OCSP_BASICRESP *bs, STACK_OF(X509) *certs, X509_STORE *st, unsigned long flags)
{
    ...
    GUARD_CLEANUP(ocsp_find_signer(&signer, bs, certs, &flags), basic_verify_cleanup(chain, bs, certs, untrusted), "S2N_OCSP_ERR_CERT_SIGNER_CERTIFICATE_NOT_FOUND");

    /* returns success if public key in signer matches basic response */
    GUARD_CLEANUP(ocsp_signer_verify(bs, signer, flags), basic_verify_cleanup(chain, bs, certs, untrusted), "S2N_OCSP_ERR_CERT_SIGNATURE_FAILURE");
    ...
    if (!IS_OCSP_FLAG_SET(flags, OCSP_NOVERIFY)) {
        ...
        GUARD_CLEANUP(ocsp_assign_untrusted(&untrusted, bs, certs, flags), basic_verify_cleanup(chain, bs, certs, untrusted), "S2N_OCSP_ERR_ALLOC");

        /*  do checks on ctx store and assign value to chain */
        GUARD_CLEANUP(ocsp_assign_chain(&chain, st, signer, untrusted), basic_verify_cleanup(chain, bs, certs, untrusted), "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
        ...
        if (!IS_OCSP_FLAG_SET(flags, OCSP_NOCHECKS)) {
            ...
            GUARD_CLEANUP(ocsp_check_issuer(bs, chain, flags), basic_verify_cleanup(chain, bs, certs, untrusted), "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
        }
    }
    GUARD(basic_verify_cleanup(chain, bs, certs, untrusted));
    return S2N_OCSP_SUCCESS;
}
                     ...
                     STACK_OF(X509) *certs, unsigned long *flags)
{
    ...
    if (ocsp_find_signer_sk(certs, rid, &signer) == S2N_OCSP_SUCCESS) {
        *psigner = signer;
    ...
    if (!IS_OCSP_FLAG_SET(*flags, OCSP_NOINTERN) &&
        (ocsp_find_signer_sk(bs->certs, rid, &signer) == S2N_OCSP_SUCCESS)) {
        *psigner = signer;
        return S2N_OCSP_SUCCESS;
    }
...
static int ocsp_find_signer_sk(STACK_OF(X509) *certs, OCSP_RESPID *id, X509 **signer)
{
    ...
    for (size_t i = 0; i < sk_X509_num(certs); i++) {
        ...
        ENSURE_POSIX(X509_pubkey_digest(x, EVP_sha1(), tmphash, NULL) > 0, "S2N_OCSP_ERR_CERT_UNKNOWN_MESSAGE_DIGEST");
        if (memcmp(keyhash, tmphash, SHA_DIGEST_LENGTH) == S2N_OCSP_SUCCESS) {
            *signer = x;
            return S2N_OCSP_SUCCESS;
        }
    }
...
static int ocsp_signer_verify(OCSP_BASICRESP *bs, X509 *signer, unsigned long flags)
{
    ...
    if (!IS_OCSP_FLAG_SET(flags, OCSP_NOSIGS)) {
        ...
        ENSURE_POSIX(skey != NULL, "S2N_OCSP_ERR_CERT_NO_SIGNER_KEY");

        /* ASN1 OpenSSL function, returns 1 or more on success, 0 or below on failure */
        ENSURE_POSIX(OCSP_BASICRESP_verify(bs, skey, 0) > 0, "S2N_OCSP_ERR_CERT_SIGNATURE_FAILURE");
    }
    return S2N_OCSP_SUCCESS;
}
                        ...
                        STACK_OF(X509) *certs, unsigned long flags)
{
    ...
    if(IS_OCSP_FLAG_SET(flags, OCSP_NOCHAIN)) {
        ...
        *untrusted = sk_X509_dup(bs->certs);
        for (size_t i = 0; i < sk_X509_num(certs); i++) {
            ENSURE_POSIX(sk_X509_push(*untrusted, sk_X509_value(certs, i)) > 0, "S2N_OCSP_ERR_ALLOC");
        }
    } else if (certs != NULL) {
        *untrusted = certs;
    } else {
...
static int ocsp_assign_chain(STACK_OF(X509) **chain, X509_STORE *st, X509 *signer, STACK_OF(X509) *untrusted)
{
    ...
    ENSURE_POSIX(X509_STORE_CTX_init(ctx, st, signer, untrusted) > 0, "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
    ENSURE_POSIX(X509_STORE_CTX_set_purpose(ctx, X509_PURPOSE_OCSP_HELPER) > 0, "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
    ENSURE_POSIX(X509_verify_cert(ctx) > 0, "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
...
/* returns success on check success, S2N_OCSP_FAILURE on failure */
static int ocsp_check_issuer(OCSP_BASICRESP *bs, STACK_OF(X509) *chain, unsigned long flags)
{
    ENSURE_POSIX_REF(bs);
    ...
    /* X509 OpenSSL function, returns 1 or more on success, 0 or below on failure */
    ENSURE_POSIX(sk_X509_num(chain) > 0, "S2N_OCSP_ERR_CERT_NO_CERTIFICATES_IN_CHAIN");
    ...
    if (sk_X509_num(chain) > 1) {
        ...
        ENSURE_POSIX(issuer_match_ret >= 0, "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
        if(issuer_match_ret == S2N_OCSP_SUCCESS){
    ...
    * if flags do not contain OCSP_NOEXPLICIT the function checks for explicit trust for OCSP signing in the root CA certificate. */
    ENSURE_POSIX(!IS_OCSP_FLAG_SET(flags, OCSP_NOEXPLICIT), "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");

    ENSURE_POSIX_REF(x = sk_X509_value(chain, sk_X509_num(chain) - 1));
    ENSURE_POSIX(X509_check_trust(x, NID_OCSP_sign, 0) == X509_TRUST_TRUSTED, "S2N_OCSP_ERR_CERT_ROOT_CA_NOT_TRUSTED");
    return S2N_OCSP_SUCCESS;
}

/* S2N_OCSP_MISMATCH on just a mismatch  S2N_OCSP_SUCCESS on match, S2N_FAILURE on failure */
static int ocsp_match_issuerid(X509 *cert, OCSP_CERTID *cid, STACK_OF(OCSP_SINGLERESP) *sresp)
{
    ...
    if(cid) {
        ...
        ENSURE_POSIX(mdlen >= 0, "S2N_OCSP_ERR_CERT_UNKNOWN_MESSAGE_DIGEST");
        iname = X509_get_subject_name(cert);

        ENSURE_POSIX(X509_NAME_digest(iname, dgst, md, NULL) > 0, "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
        ...
        if((cid->issuerNameHash->length != mdlen) || (cid->issuerKeyHash->length != mdlen)) {
            return S2N_OCSP_MISMATCH;
        }
        /* ensure that memcmp equals 0, if not, it's a mismatch */
        if(memcmp(md, cid->issuerNameHash->data, mdlen) != S2N_OCSP_SUCCESS) {
            return S2N_OCSP_MISMATCH;
        }
        /* X509 OpenSSL function, returns 1 or more on success, 0 or below on failure */
        ENSURE_POSIX(X509_pubkey_digest(cert, dgst, md, NULL) > 0, "S2N_OCSP_ERR_CERT_UNKNOWN_MESSAGE_DIGEST");
        /* ensure that memcmp equals 0, if not, it's a mismatch */
        if(memcmp(md, cid->issuerKeyHash->data, mdlen) != S2N_OCSP_SUCCESS) {
            return S2N_OCSP_MISMATCH;
        }
...
static int ocsp_check_ids(STACK_OF(OCSP_SINGLERESP) *sresp, OCSP_CERTID **ret)
{
    ...
    ENSURE_POSIX(idcount > 0, "S2N_OCSP_ERR_CERT_RESPONSE_CONTAINS_NO_REVOCATION_DATA");
    ...
    for (size_t i = 1; i < idcount; i++) {
        ...
        if(OCSP_id_issuer_cmp(cid, tmpid)) {
            ...
            ENSURE_POSIX(!OBJ_cmp(tmpid->hashAlgorithm->algorithm, cid->hashAlgorithm->algorithm),
                         "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
...
static int ocsp_check_delegated(X509 *x)
{
    ...
    ENSURE_POSIX((X509_get_extension_flags(x) & EXFLAG_XKUSAGE) && (X509_get_extended_key_usage(x) & XKU_OCSP_SIGN),
            "S2N_OCSP_ERR_CERT_MISSING_OCSPSIGNING_USAGE");
    return S2N_OCSP_SUCCESS;
}
