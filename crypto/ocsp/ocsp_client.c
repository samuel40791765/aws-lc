...
int OCSP_response_status(OCSP_RESPONSE *resp, int64_t *resp_status)
{
  ...
  ENSURE_POSIX(0 <= status, "S2N_OCSP_ERR_INTEGER_OVERFLOW");
  *resp_status = status;
...
int OCSP_response_get1_basic(OCSP_RESPONSE *resp, OCSP_BASICRESP **basic_resp)
{
  ...
  ENSURE_POSIX((OBJ_obj2nid(rb->responseType) == NID_id_pkix_OCSP_basic), "S2N_OCSP_ERR_CERT_NOT_BASIC_RESPONSE");
...
int OCSP_resp_count(OCSP_BASICRESP *bs, int *count)
{
  ...
  *count = sk_OCSP_SINGLERESP_num(bs->tbsResponseData->responses);
  ENSURE_POSIX(*count != -1, "S2N_OCSP_ERR_NULL");
  return S2N_OCSP_SUCCESS;
}
...
int OCSP_resp_get0(OCSP_BASICRESP *bs, int idx, OCSP_SINGLERESP **single_resp)
{
  ...
  ENSURE_POSIX(0 <= idx && idx < resp_count ,"S2N_OCSP_ERR_ARRAY_INDEX_OOB");
...
int OCSP_basic_add1_cert(OCSP_BASICRESP *bs, X509 *cert)
{
  ...
  ENSURE_POSIX(sk_X509_push(bs->certs, cert) > 0,"S2N_OCSP_ERR_SIZE_MISMATCH");
  ENSURE_POSIX( X509_up_ref(cert) > 0, "S2N_OCSP_ERR_SIZE_MISMATCH");
  return S2N_OCSP_SUCCESS;
}
                          ...
                          ASN1_GENERALIZEDTIME **nextupd)
{
  ...
  ENSURE_POSIX(idx >= 0, "S2N_OCSP_ERR_CERT_CERTIFICATE_VERIFY_ERROR");
                     ...
                     OCSP_CERTID **new_certid)
{
  ...
  ENSURE_CERTID_CLEANUP( (cid = OCSP_CERTID_new()) != NULL, OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_UNKNOWN_NID");
  alg = cid->hashAlgorithm;
  ASN1_OBJECT_free(alg->algorithm);
  ENSURE_CERTID_CLEANUP( (nid = EVP_MD_type(dgst)) != NID_undef, OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_UNKNOWN_NID");

  ENSURE_CERTID_CLEANUP( 0 < X509_ALGOR_set0(alg, OBJ_nid2obj(nid), V_ASN1_NULL, NULL), OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_UNKNOWN_NID");
  ENSURE_CERTID_CLEANUP( X509_NAME_digest(issuerName, dgst, md, &i), OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_DIGEST_ERR");
  ENSURE_CERTID_CLEANUP( ASN1_OCTET_STRING_set(cid->issuerNameHash, md, i), OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_DIGEST_ERR");

  /* Calculate the issuerKey hash, excluding tag and length */
  ENSURE_CERTID_CLEANUP( EVP_Digest(issuerKey->data, issuerKey->length, md, &i, dgst, NULL), OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_DIGEST_ERR");
  ENSURE_CERTID_CLEANUP( ASN1_OCTET_STRING_set(cid->issuerKeyHash, md, i), OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_DIGEST_ERR");
  ENSURE_CERTID_CLEANUP( EVP_Digest(issuerKey->data, issuerKey->length, md, &i, dgst, NULL), OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_DIGEST_ERR");

  if (serialNumber != NULL) {
    ENSURE_CERTID_CLEANUP( ASN1_STRING_copy(cid->serialNumber, serialNumber) != 0, OCSP_cert_id_new_cleanup(cid) , "S2N_OCSP_ERR_CERT_DIGEST_ERR");
  }
  *new_certid = cid;
  return S2N_OCSP_SUCCESS;
}
