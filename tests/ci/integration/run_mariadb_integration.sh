...
MARIADB_BUILD_FOLDER="${SCRATCH_FOLDER}/mariadb-aws-lc"
MARIADB_INSTALL_FOLDER="${SCRATCH_FOLDER}/mariadb-aws-lc-install"
...
function mariadb_build() {
  cmake ${MARIADB_SRC_FOLDER} -GNinja -DWITH_SSL=${AWS_LC_INSTALL_FOLDER} "-B${MARIADB_BUILD_FOLDER}" -DCMAKE_INSTALL_PREFIX="${MARIADB_INSTALL_FOLDER}" -DPLUGIN_COLUMNSTORE=NO -DPLUGIN_ROCKSDB=NO -DPLUGIN_S3=NO -DPLUGIN_MROONGA=NO -DPLUGIN_CONNECT=NO -DPLUGIN_SPHINX=NO -DPLUGIN_SPIDER=NO -DPLUGIN_TOKUDB=NO -DPLUGIN_PERFSCHEMA=NO -DWITH_WSREP=OFF -DCMAKE_BUILD_TYPE=RELEASE
