...
MYSQL_INSTALL_FOLDER="${SCRATCH_FOLDER}/mysql-aws-lc-install"
...
function mysql_build() {
  cmake ${MYSQL_SRC_FOLDER} -GNinja -DWITH_NDB_JAVA=OFF  -DWITH_BOOST=${BOOST_INSTALL_FOLDER} -DWITH_SSL=${AWS_LC_INSTALL_FOLDER} "-B${MYSQL_BUILD_FOLDER}" -DCMAKE_INSTALL_PREFIX="${MYSQL_INSTALL_FOLDER}" -DCMAKE_BUILD_TYPE=RELEASE
